// Continue sync by navigating from page 1
  ...
  try {
    ...
    while (currentPage <= maxPages) {
      ...
      try {
        ...
        // Stop if we've gone 10 pages without new data (means we reached the end)
        if (newInPage === 0) {
          // Check if this is because of duplicates or end of pages
          // If total in DB equals expected (~4557), we're done
          if (dbTotal >= 4500) {
            console.log('[SYNC] Reached expected total, stopping');
            break;
          }
        }
        ...
        // Refresh browser every 50 pages
        if (currentPage % 50 === 1 && currentPage > 1) {
          ...
          // Update reference
          browser.close = () => newBrowser.close();
          const newPage = await newBrowser.newPage();
          await newPage.goto('https://sipp.pn-natuna.go.id/', { waitUntil: 'domcontentloaded', timeout: 30000 });
          await newPage.waitForSelector('table', { timeout: 10000 });
          await sleep(2000);

          // Navigate to current page
          for (let i = 1; i < currentPage; i++) {
            await newPage.evaluate((pageNum) => {
              const pagesDiv = document.getElementById('pages');
              const pageLink = Array.from(pagesDiv.querySelectorAll('a')).find(a =>
                a.textContent.trim() === (pageNum + 1).toString()
              );
